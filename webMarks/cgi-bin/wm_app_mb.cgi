...
sub main_func
{
	...
	if(defined($q_parm) && ($q_parm eq 'auth'))
	{
		...
		{
			GenMarks_mb->new()->genDefaultPage( Error->new(112) );		
		}
	}
	...
	elsif(defined($q_parm) && ($q_parm eq 'regAuth'))
	{
		...
		else
		{
			...
			if ($@) 
			{
				...
				GenMarks_mb->new()->genRegistration(Error->new(120));
			}
			else
			{
...
#=cut
    ...
    if ($@) 
    {
	   $genMarks = GenMarks_mb->new($tabMap{tabtype},undef,undef,Error->new(2000));	
